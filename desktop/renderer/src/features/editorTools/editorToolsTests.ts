...
import { SYNTAX_THEME_LIST, getSyntaxTheme, themeCssVars } from './syntax/syntaxThemes';
...
// 9. Syntax themes
check('theme count', SYNTAX_THEME_LIST.length === 4);
check('paper heading color', getSyntaxTheme('paper').palette.heading === '#5a3e2b');
check('themeCssVars has --syn-heading', themeCssVars('paper')['--syn-heading'] === '#5a3e2b');
check(
  'theme switching changes colors',
  themeCssVars('writer-dark')['--syn-heading'] !== themeCssVars('paper')['--syn-heading'],
);
check('unknown theme falls back to minimal', getSyntaxTheme('nope' as never).id === 'minimal');
